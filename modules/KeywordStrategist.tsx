                ...
                <div className="lg:col-span-2">
                    {isLoading ? (
                        ...
                        <div className="space-y-6 animate-slide-in-up">
                                                ...
                                                {cluster.keywords.map((kw, j) => (
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${kw.difficulty === 'High' ? 'bg-red-100 text-red-800' :
                            ...
                            {strategy.zeroClickOpportunities && strategy.zeroClickOpportunities.length > 0 && (
                                <Section title="Zero-Click Intelligence" description="Questions you can answer directly to capture SERP real estate.">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {strategy.zeroClickOpportunities.map((opp, i) => (
                                            <Card key={i} className="p-4 bg-indigo-50 border-indigo-100">
                                                <div className="flex items-start gap-3">
                                                    <div className="mt-1 size-5 bg-indigo-500 rounded-full flex items-center justify-center text-[10px] text-white font-bold">?</div>
                                                    <div>
                                                        <h4 className="text-sm font-bold text-indigo-900">{opp.question}</h4>
                                                        <p className="text-xs text-indigo-700/70 mt-1 italic">Target: {opp.keyword}</p>
                                                        <div className="mt-3 p-3 bg-white rounded-lg border border-indigo-200 text-xs text-gray-600 font-serif line-clamp-3">
                                                            {opp.answerSnippet}
                                                        </div>
                                                        <button
                                                            onClick={() => navigateToModule('content-creator', { topic: `Answer Zero-Click Question: ${opp.question}`, context: { keyword: opp.keyword, mode: 'qa' } })}
                                                            className="mt-3 text-[10px] font-bold text-indigo-500 uppercase tracking-widest hover:text-indigo-700 transition-colors"
                                                        >
                                                            Generate Q&A Asset
                                                        </button>
                                                    </div>
