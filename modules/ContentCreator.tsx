            ...
            switch (mode) {
                case 'post':
                    ...
                    if (postResponse.imagePrompt) {
                        ...
                        try {
                            const imageB64 = await geminiService.generateImage(postResponse.imagePrompt);
    ...
    const handleRefinePost = async () => {
        ...
        try {
            ...
            if (refinedPost.imagePrompt !== originalPost.imagePrompt) {
                setGeneratedImage({ url: '', isLoading: true });
                 try {
                    const imageB64 = await geminiService.generateImage(refinedPost.imagePrompt);
                    setGeneratedImage({ url: `data:image/png;base64,${imageB64}`, isLoading: false });
                } catch (imgError) {
            ...
            return (
                <div className="text-center p-8 animate-slide-in-up space-y-4">
                    <div className="inline-block p-3 bg-green-500/20 rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-8 w-8 text-green-400"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                    </div>
